...
rclcpp_action::GoalResponse
    ...
    bool use_goal_pose_with_tf = camera_link0_tf_buffer->canTransform(
        "base_link", "camera_optical_frame", tf2::TimePointZero, tf2::durationFromSec(1.0));
    try {
        if (use_goal_pose_with_tf) {
            camera_link0_tf = camera_link0_tf_buffer->lookupTransform("base_link", "camera_optical_frame", tf2::TimePointZero);
        }
    } catch (const tf2::TransformException& ex) {
        use_goal_pose_with_tf = false;
    }

    if (use_goal_pose_with_tf) {
        // 将相机识别的物体位置转换到主坐标系下，保存到 task_target_pos
        ...
        // TF 暂不可用时，退化为使用 box_pose 话题中最近一次 base_link 位姿。
        std::lock_guard<std::mutex> lock(vision_target_mutex_);
        if (!has_vision_target_) {
            RCLCPP_WARN(node->get_logger(), "警告：相机坐标系和变换查询失败，且无可用 box_pose 缓存，拒绝机械臂目标请求");
            return rclcpp_action::GoalResponse::REJECT;
        }
        task_target_pos = detected_target_pose_on_base_link_;
        RCLCPP_WARN(node->get_logger(), "相机TF查询失败，使用 box_pose 缓存位姿继续执行抓取");
    }
...
void ArmHandleNode::arm_catch_task_handle() {
    ...
    // geometry_msgs::msg::Pose experimental_pose_ ;
    // experimental_pose_.position.x = 0.5;
    // experimental_pose_.position.y = 0.0;
    // experimental_pose_.position.z = 0.0;
    // experimental_pose_.orientation.w = 0.7071;
    // experimental_pose_.orientation.x = 0.0;
    // experimental_pose_.orientation.y = -0.7071;
    // experimental_pose_.orientation.z = 0.0;
    // // add_kfs_collision(experimental_pose_, "target_kfs", move_group_interface->getPlanningFrame());

    // move_group_interface->setPoseTarget(experimental_pose_);
    // success = (move_group_interface->plan(plan) == moveit::core::MoveItErrorCode::SUCCESS);
    // if(success){
    //     std::cout << "===================================================================================================" << std::endl;
    //     std::cout << "[DEBUG 1] 实验位置可达" << std::endl;
    //     std::cout << "===================================================================================================" << std::endl;
    // } else {
    //     std::cout << "===================================================================================================" << std::endl;
    //     std::cout << "[DEBUG 1] 实验位置不可达" << std::endl;
    //     std::cout << "===================================================================================================" << std::endl;
    // };
    ...
    while (rclcpp::ok()) {
        ...
        if (current_task_type == ROBOTIC_ARM_TASK_MOVE)           // 要求机械臂移动到某个位姿（因为移动动作在一个周期内完成，所以不再需要执行）
        {
            ...
            //     (target_pose_on_base_link.position.z >= -0.05 && target_pose_on_base_link.position.z <= 1);
            ...
            // 兜底位姿：采用更保守的位置与当前末端朝向，降低目标采样失败概率
            {
                ...
                fallback_prepare_pos.orientation = prepare_pos.orientation;
            }
