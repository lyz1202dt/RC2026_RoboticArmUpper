...
// Keep arm_calc visualization output off global /joint_states to avoid interfering with MoveIt state monitoring.
constexpr char kRvizJointTopic[] = "arm_calc/joint_states";
...
std::string ArmCtrlNode::load_local_urdf() const {
    const std::string arm_share = ament_index_cpp::get_package_share_directory("robotic_arm");
    const std::string urdf_path = arm_share + "/urdf/robotic_arm_mujoco.urdf";
...
JointState ArmCtrlNode::from_arm_message(const robot_interfaces::msg::Arm& msg) {
    JointState state;
    for (std::size_t i = 0; i < kJointDoF; ++i) {
        state.position[static_cast<int>(i)] = msg.joints[i].rad;
        state.velocity[static_cast<int>(i)] = msg.joints[i].omega;
        state.torque[static_cast<int>(i)] = msg.joints[i].torque;
    }
    return state;
}
...
robot_interfaces::msg::Arm ArmCtrlNode::to_arm_message(const JointTrajectoryPoint& point) {
    ...
    for (std::size_t i = 0; i < kJointDoF; ++i) {
        msg.joints[i].rad = static_cast<float>(point.position[static_cast<int>(i)]);
        msg.joints[i].omega = static_cast<float>(point.velocity[static_cast<int>(i)]);
        msg.joints[i].torque = static_cast<float>(point.torque[static_cast<int>(i)]);
    }
    return msg;
}
